...
type PrometheusSpec struct {
	...
	// (Alpha) Using this field requires the ShardRetentionPolicies feature gate to be enabled.
	//
...
type WhenScaledDownRetentionType string

var (
	WhenScaledDownRetentionTypeRetain WhenScaledDownRetentionType = "Retain"
	WhenScaledDownRetentionTypeDelete WhenScaledDownRetentionType = "Delete"
)

type ShardRetentionPolicy struct {
	// WhenScaledDown defines the retention policy when the Prometheus Server is scaled-down.
	// When set to retain, the shard will only stop scraping targets and will retain the data.
	// The targets scraped by the scaled-down shard(s) are redistributed to the remaining shards.
	//
	// Default: "Delete"
	// +kubebuilder:validation:Enum=Retain;Delete
	// +required
	WhenScaledDown WhenScaledDownRetentionType `json:"whenScaledDown"`
}
